"""
plugin_manager.py: Main plugin manager for LabKit
"""
import os
import sys
...
from .plugin_infra.interfaces import CallablePlugin, HookPlugin, UtilityPlugin, CommandPlugin, EventPlugin


class PluginManager:
    """
    PluginManager: Main plugin manager for LabKit
    """
    ...
    def __init__(self, plugins_dir: Optional[Path] = None):
        ...
        # Plugin registries
        self.callable_plugins: Dict[str, CallablePlugin] = {}
        self.hook_plugins: Dict[str, HookPlugin] = {}
        self.command_plugins: Dict[str, CommandPlugin] = {}
        self.event_plugins: Dict[str, EventPlugin] = {}
        self.utility_plugins: Dict[str, UtilityPlugin] = {}
        
        # Infrastructure components
        self.fs_writer = None
        self.timer_trigger = None

    def load_infrastructure(self, config: Dict[str, Any]):
        """
        Load infrastructure components
        :param config: Configuration for infrastructure
        """
        from .plugin_infra.fs_writer import SecureFSWriter
        from .plugin_infra.timer import TimerTrigger
        
        # Initialize secure filesystem writer
        fs_config = config.get("fs_writer", {})
        # Also check for plugin_infra.fs_writer configuration
        plugin_infra_config = config.get("plugin_infra", {})
        if "fs_writer" in plugin_infra_config:
            # Merge the configurations, with plugin_infra taking precedence
            from typing import Dict
            if isinstance(fs_config, Dict) and isinstance(plugin_infra_config["fs_writer"], Dict):
                import copy
                merged_config = copy.deepcopy(fs_config)
                merged_config.update(plugin_infra_config["fs_writer"])
                fs_config = merged_config
            else:
                fs_config = plugin_infra_config["fs_writer"]
        ...
        try:
            ...
            for attr_name in dir(module):
                ...
                if (isinstance(attr, type) and
                    attr != object and
                    issubclass(attr, (CallablePlugin, HookPlugin, CommandPlugin, UtilityPlugin, EventPlugin))):
                    # Skip abstract base classes
                    import abc
                    if (hasattr(attr, '__abstractmethods__') and attr.__abstractmethods__) or abc.ABC in attr.__bases__:
            ...
            if "config" in manifest:
                plugin_config.update(manifest["config"])
            ...
            if hasattr(plugin_instance, 'init') and callable(getattr(plugin_instance, 'init')):
                success = plugin_instance.init(plugin_config, **init_kwargs)
                if not success:
                    self.logger.error(f"Plugin {plugin_instance.get_name()} failed to initialize")
                    return None
            else:
                # If no init method, try to set properties directly
            ...
            if plugin_type == "callable":
                ...
                self.command_plugins[plugin_name] = plugin_instance
            ...
            self.logger.error(f"Error loading plugin from {plugin_dir}: {e}", exc_info=True)
            return None
    ...
    def load_all_plugins(self, config: Dict[str, Any]) -> Dict[str, Any]:
        ...
        self.load_infrastructure(config)
    ...
    def get_hook_plugins(self) -> Dict[str, Any]:
        """Get all hook plugins"""
        return self.hook_plugins.copy()

    def get_command_plugins(self) -> Dict[str, Any]:
        """Get all command plugins"""
        return self.command_plugins.copy()
    ...
    def get_plugin(self, name: str) -> Optional[Any]:
        ...
        for plugin_dict in [self.callable_plugins, self.hook_plugins, self.command_plugins, self.event_plugins, self.utility_plugins]:
    ...
    def execute_callable_plugin(self, name: str, *args, **kwargs) -> Optional[Any]:
        ...
        try:
            ...
            self.logger.error(f"Error executing callable plugin '{name}': {e}", exc_info=True)
            return None
    ...
    def handle_event(self, event: Dict[str, Any]) -> List[bool]:
        ...
        """
        results = []
        for plugin_name, plugin in self.event_handler_plugins.items():
            ...
            if event_type in event_types or action in plugin.get_actions():
                try:
                    ...
                    self.logger.error(f"Error in {plugin_name} handling event: {e}", exc_info=True)
                    results.append(False)
            else:
                # Plugin is not interested in this event
                continue
    ...
    def cleanup(self):
        ...
        # Cleanup plugins
        all_plugins = {**self.callable_plugins, **self.hook_plugins, **self.command_plugins, **self.event_plugins, **self.utility_plugins}
        for name, plugin in all_plugins.items():
            if hasattr(plugin, 'cleanup') and callable(getattr(plugin, 'cleanup')):
                try:
                    plugin.cleanup()
                except Exception as e:
                    self.logger.error(f"Error cleaning up plugin {name}: {e}", exc_info=True)

        # Cleanup infrastructure components if they have cleanup methods
        if self.timer_trigger and hasattr(self.timer_trigger, 'cleanup'):
            try:
                self.timer_trigger.cleanup()
            except Exception as e:
                self.logger.error(f"Error cleaning up timer trigger: {e}", exc_info=True)